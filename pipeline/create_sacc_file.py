...
def main(args):
        ...
        for j, (ms3, ms4) in enumerate(ps_names):
            ...
            for ifp1, fp1 in enumerate(field_pairs):
                for ifp2, fp2 in enumerate(field_pairs):
                    cov[
                        ifp1 * len(lb) : (ifp1 + 1) * len(lb),
                        ifp2 * len(lb) : (ifp2 + 1) * len(lb),
                    ] = cov_dict[fp1 + fp2]
    ...
    for id_sim in mpi.taskrange(Nsims - 1):
            ...
            for spin, qty in zip([0, 2], ["cmb_temperature", "cmb_polarization"]):
                ...
                s.add_tracer(
                    **{
                        ...
                        "quantity": qty,
                        "spin": spin,
                        "nu": [f - 1.0, f, f + 1],  # Delta bandpasses. TODO: generalize
                        "ell": lb,
                        "beam": np.ones_like(lb),  # Unit beam. TODO: generalize
                        "bandpass": [0.0, 1.0, 0.0],  # Deltas. TODO: generalize
                    }
                )
        ...
        for i, (ms1, ms2) in enumerate(ps_names):
            ...
            cl_file = (
                f"{cl_dir}/decoupled_cross_pcls_{ms1}_x_{ms2}{sim_label}.npz"  # noqa
            )
