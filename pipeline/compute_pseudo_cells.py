...
def main(args):
    ...
    for map_name in meta.maps_list:
        ...
        type_options = [f for f in re.findall(r"\{.*?\}", map_template) if "|" in f][
            0
        ]  # noqa
        # Select the hitmap
        ...
        if do_plots:
            for i, f in enumerate(["T", "Q", "U"]):
                ...
                plt.savefig(
                    f"{map_plot_dir}/map_{map_set}_" f"bundle{id_bundle}_{f}.png"
                )
                plt.close()
    ...
    for map_name1, map_name2 in meta.get_ps_names_list(type="all", coadd=False):
        ...
        pcls = pu.get_coupled_pseudo_cls(
            fields[map_set1, id_bundle1], fields[map_set2, id_bundle2], nmt_bins
        )
        ...
        if do_plots:
            ...
            for i, fp in enumerate(field_pairs):
                ...
                plt.title(
                    f"({map_set1}, bundle {id_bundle1}) "
                    f"x ({map_set2}, bundle {id_bundle2}) | {fp}"
                )
                plt.loglog(
                    lb,
                    cb2db * decoupled_pcls[fp],
                    c="navy",
                    marker="o",
                    mfc="w",
                    ls="",
                ...
                plt.loglog(ll, cl2dl * clth, c="darkorange", ls="--", label="Theory")
                plt.ylabel(r"$D_\ell$", fontsize=15)
                plt.xlabel(r"$\ell$", fontsize=15)
                plt.legend(fontsize=13)
                plt.savefig(
                    f"{cl_plot_dir}/pcls_{map_set1}_"
