...
def decouple_pseudo_cls(coupled_pseudo_cells, coupling_inv):
    ...
    Parameters
    ----------
    coupled_pseudo_cells : dict with keys f"spin{s1}xspin{s2}", items array-like
...
def field_pairs_from_spins(cls_in_dict):
    """
    Reorders power spectrum dictionary with a given input spin pair into pairs 
    of output (pseudo-)scalar fields on the sky (T, E, or B).
    Parameters
    ----------
...
def get_pcls_mat_transfer(fields, nmt_binning):
    """
    Compute coupled binned pseudo-C_ell estimates from pure-E and pure-B 
    transfer function estimation simulations, and cast them into matrix shape.
    Parameters
    ----------
    ...
    for index, (pure_type1, pure_type2) in enumerate(product(cases, cases)):
        pcls = get_coupled_pseudo_cls(fields[pure_type1], fields[pure_type2], 
                                      nmt_binning)
...
def pcler(args):
    """
    Compute all decoupled binned pseudo-C_ell estimates needed for a specific 
    task in the pipeline related to simulations or data.
