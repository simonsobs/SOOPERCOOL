...
from sotodlib.tod_ops import apodize, detrend_tod, fft_ops, filters, flags, sub_polyf
...
def generate_gaussian_map(ps_dict, map_shape, wcs, seed=1234):
    ...
    return enmap.rand_map(
        (3, map_shape[0], map_shape[1]), wcs, cov, spin=[0, 2], seed=seed
    )
...
def get_aman(context, obs_id, wafer, freq, thinning_factor=1.0, seed=1234):
    ...
    # Select wafer and frequency
    # (remember atomic maps are saved per wafer and per frequency)
...
def filter_sim(aman, input_map, wcs, return_nofilter=False):
    ...
    dsT_sim, demodQ_sim, demodU_sim = demod_mm.from_map(aman, input_map, wrap=True)
    ...
    if return_nofilter:
        return (
            res_filt["weighted_map"],
            res_filt["weight"],
            res_nofilt["weighted_map"],
            res_nofilt["weight"],
        )
...
def calibrate_obs_tomoki(
    ...
    obs.restrict(
        "dets", obs.dets.vals[(0.2 < obs.det_cal.r_frac) & (obs.det_cal.r_frac < 0.8)]
    )
    ...
    if obs.signal is not None:
        ...
        speed = (
            np.sum(np.abs(np.diff(np.unwrap(obs.hwp_angle))))
            / (obs.timestamps[-1] - obs.timestamps[0])
        ) / (2 * np.pi)
        bpf_center = 4 * speed
        bpf_width = speed * 2.0 * 0.9
        bpf_cfg = {
            "type": "sine2",
            "center": bpf_center,
            "width": bpf_width,
        ...
        lpf_cfg = {"type": "sine2", "cutoff": lpf_cutoff, "trans_width": 0.1}
        ...
        freq, Pxx_demodQ = fft_ops.calc_psd(
            obs, signal=obs.demodQ, nperseg=nperseg, merge=False
        )
        freq, Pxx_demodU = fft_ops.calc_psd(
            obs, signal=obs.demodU, nperseg=nperseg, merge=False
        )
        ...
        freq, Pxx_demodQ_new = fft_ops.calc_psd(
            obs, signal=obs.demodQ, nperseg=nperseg, merge=False
        )
        freq, Pxx_demodU_new = fft_ops.calc_psd(
            obs, signal=obs.demodU, nperseg=nperseg, merge=False
        )
        ...
        for di, det in enumerate(obs.dets.vals):
            ...
            popt, pcov = curve_fit(
                ...
                x,
                np.log(y),
                p0=(np.sqrt(np.median(y[x > 0.2])), 0.01, -2.0),
                maxfev=100000,
            )
            obs.sigma[di] = popt[0]
            obs.fk[di] = popt[1]
        ...
        #     np.logical_or(
        #         obs.flags["left_scan"].mask(), obs.flags["right_scan"].mask()
        #     ),
        ...
        badsubscan_indicator = (
            (np.abs(subscan_Qkurt) > kurt_threshold)
        ...
        wn = fft_ops.calc_wn(obs, obs.Pxx_demodQ, low_f=0.1, high_f=1.0)
