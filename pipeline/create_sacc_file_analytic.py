...
def main(args):
    ...
    if args.cov_sigma_scale <= 0:
        raise ValueError("--cov-sigma-scale must be positive.")
    full_cov *= args.cov_sigma_scale**2
    if verbose:
        print(
            f"Scaling stored covariance by {args.cov_sigma_scale**2:.6e} "
            f"so sigma is scaled by {args.cov_sigma_scale:.6e}."
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=1e6,
        help=(
            "Multiplicative factor applied to covariance sigmas. "
            "The stored covariance is multiplied by this value squared."
        )
    )
