...
def main(args):
    ...
    field_spin0 = nmt.NmtField(mask, None, wcs=wcs, spin=0)
    field_spin2 = nmt.NmtField(mask, None, wcs=wcs, spin=2, purify_b=meta.pure_B)
    ...
    mcm = np.transpose(
        w.get_coupling_matrix().reshape([nl, nspec, nl, nspec]), axes=[1, 0, 3, 2]
    )
    ...
    for map_set in meta.map_sets_list:
        ...
        _, bl = su.read_beam_from_file(f"{beam_dir}/{beam_file}", lmax=lmax)
        beams[map_set] = bl

    # Beam-correct the mode coupling matrix
    beamed_mcm = {}
    for map_set1, map_set2 in meta.get_ps_names_list("all", coadd=True):
        beamed_mcm[map_set1, map_set2] = (
            mcm
            * np.outer(beams[map_set1], beams[map_set2])[np.newaxis, :, np.newaxis, :]
        )
