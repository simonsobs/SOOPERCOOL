...
def plot_spectrum(
    ...
    if add_theory:
        ...
        main.plot(lth, fac_th * clth, c="darkgray", ls="-.", lw=2.6, label="theory")
...
def main(args):
    ...
    for ms1, ms2 in ps_names:
        ...
        for i, fp in enumerate(["TT", "EE", "BB", "TE"]):
            ...
            dust_cl = hp.read_cl(fiducial_dust.format(nu1=nu1, nu2=nu2))[i, : lmax + 1]
        ...
        for fp in field_pairs:
            ...
            mask = (np.arange(len(lb)) > idx_bad) & (lb <= 2 * meta.nside - 1)
        ...
        for ms1, ms2 in ps_names:
            ...
            for fp in field_pairs:
                ...
                ell, cl, cov = s.get_ell_cl(
                    f"cl_{types[f1]}{types[f2]}", ms1, ms2, return_cov=True
                )

                idx_bad = idx_bad_tf[ftag1, ftag2][fp]
                mask = (np.arange(len(ell)) > idx_bad) & (ell <= 2 * meta.nside - 1)
    ...
    for id_sim in range(Nsims):
        ...
        s = sacc.Sacc.load_fits(f"{sacc_dir}/cl_and_cov_sacc_{id_sim:04d}.fits")
    ...
    for ms1, ms2 in ps_names:
        for fp in field_pairs:
            ...
            plot_sims[ms1, ms2, fp]["y"] = np.mean(plot_sims[ms1, ms2, fp]["y"], axis=0)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--globals", type=str, help="Path to the global configuration file"
    )
