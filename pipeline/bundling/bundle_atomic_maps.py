...
def group_list(list, n_groups, seed=1234):
    ...
    return {id_group: list[ids_list == id_group] for id_group in ids_groups}, ids_list


def coadd_maps(map_list, res=10):
    """ """
    template_geom = enmap.band_geometry(
        (np.deg2rad(-75), np.deg2rad(25)), res=np.deg2rad(10 / 60)
    )
...
def main(args):
    ...
    for ftag in freq_tags:
        fname_list[ftag] = build_map_list(args.map_dir, ftag, type="wmap", wafer="all")
        print(f"Found maps for {ftag}: {len(fname_list[ftag])}")

    # Per-wafer
    for id_waf in range(7):
        for ftag in freq_tags:
            fname_list[ftag, id_waf] = build_map_list(
                args.map_dir, ftag, type="wmap", wafer=str(id_waf)
            )
            print(
                f"Found maps for {ftag} in wafer {id_waf}: \
    ...
    # Build a mask
    box_edges = [[-108, -180, -30, 9], [180, 144, -30, 9]]
    ...
    for ftag in freq_tags:
        for ftag2 in freq_tags:
            if ftag == ftag2:
                continue
            keep[ftag] = [
                f for f in keep[ftag] if f.replace(ftag, ftag2) in keep[ftag2]
            ]

        for id_waf in range(7):
            keep[ftag, id_waf] = [f for f in keep[ftag, id_waf] if f in keep[ftag]]

    for ftag in freq_tags:
        print(f"Found {len(keep[ftag])} maps for {ftag} (after RA/DEC cuts)")
        for id_waf in range(7):
            print(
                f"Found {len(keep[ftag, id_waf])} maps for {ftag} \
    ...
    for ftag in freq_tags:
        for id_bundle in range(n_bundles):
            to_coadd_full[ftag, id_bundle] = np.concatenate(
                [to_coadd[ftag, id_waf][id_bundle] for id_waf in range(7)]
            )
    ...
    for ftag in freq_tags:
        for id_bundle in range(n_bundles):
            ...
            plot_hit = enplot.get_plots(
                coadded_hits[ftag, id_bundle], range=[100000], colorbar=1, ticks=5
            )[0]
            enplot.write(
                f"{plot_dir}/coadd_{ftag}_bundle{id_bundle}_allwaf_hits.png", plot_hit
            )
            file_name = (
                f"{maps_dir}/TQU_CAR_coadd_{ftag}_bundle{id_bundle}_allwaf.fits"  # noqa
            )
            mu.write_map(file_name, coadded_maps[ftag, id_bundle], pix_type="car")
            mu.write_map(
            ...
            mu.write_map(
                file_name.replace("TQU", "hits"),
                coadded_hits[ftag, id_bundle],
                pix_type="car",
            )

            with open(
                f"{maps_dir}/atomic_map_list_{ftag}_bundle{id_bundle}.txt", "w"
            ) as f:  # noqa
            ...
            for id_waf in range(7):
                ...
                for field, plot in zip("TQU", plots):
                    enplot.write(
                        f"{plot_dir}/coadd_{ftag}_bundle{id_bundle}_waf{id_waf}_{field}.png",
                        plot,
                    )  # noqa
                # Plot hits
                plot_hit = enplot.get_plots(
                ...
                enplot.write(
                    f"{plot_dir}/coadd_{ftag}_bundle{id_bundle}_waf{id_waf}_hits.png",
                    plot_hit,
                )  # noqa
                ...
                with open(
                    f"{maps_dir}/atomic_map_list_{ftag}_waf{id_waf}_bundle{id_bundle}.txt",
                    "w",
                ) as f:  # noqa
    ...
    for ftag in freq_tags:
        for id_bundle in range(n_bundles):
            ...
            mu.write_map(
                f"{maps_dir}/coadd_{ftag}_bundle{id_bundle}_map.fits",
                TQU_hp,
                dtype=np.float32,
                pix_type="hp",
            )
            ...
            for id_waf in range(7):
                ...
                mu.write_map(
                    f"{maps_dir}/coadd_{ftag}_wafer{id_waf}_bundle{id_bundle}_map.fits",  # noqa
                    TQU_hp,
                    dtype=np.float32,
                    pix_type="car",
                )
                hp.write_map(
                    f"{maps_dir}/coadd_{ftag}_wafer{id_waf}_bundle{id_bundle}_hits.fits",  # noqa
                    hits_hp,
                    dtype=np.float32,
                    pix_type="car",
                )
