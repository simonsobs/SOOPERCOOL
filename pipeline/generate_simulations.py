...
def main(args):
    ...
    binary = mu.read_map(f"{masks_dir}/binary_mask.fits", pix_type=meta.pix_type)
    ...
    for id_sim in mpi.taskrange(meta.covariance["cov_num_sims"] - 1):
        ...
        for ms in meta.map_sets_list:
            ...
            cmb = mu.read_map(
                f"{signal_map_dir}/{fname}", field=[0, 1, 2], pix_type=meta.pix_type
            )
            for id_bundle in range(meta.n_bundles_from_map_set(ms)):
                ...
                noise = mu.read_map(
                    f"{noise_map_dir}/{fname}", field=[0, 1, 2], pix_type=meta.pix_type
                )
                ...
                mu.write_map(
                    f"{base_dir}/{map_name}", split_map * binary, dtype=np.float32
                )
                ...
                if do_plots:
                    ...
                    for i, f in enumerate("TQU"):
                        ...
                        plt.savefig(
                            f"{base_dir}/cov_sims_{ms}_bundle{id_bundle}_{f}.png"
                        )  # noqa
                        plt.clf()
                        plt.close()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--no-plots", action="store_true", help="Do not plot the maps."
    )  # noqa
