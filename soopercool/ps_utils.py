...
def get_validation_power_spectra(meta, id_sim, mask, nmt_binning, inv_couplings):
    """
    ...
    """
    map_set_pairs = (
        meta.get_ps_names_list(type="all", coadd=True)
        if meta.validate_beam
        else [(None, None)]
    )
    filter_flags = ["filtered"] if meta.validate_beam else ["filtered", "unfiltered"]

    for cl_type in ["tf_val", "cosmo"]:
        for filter_flag in filter_flags:
            for map_sets in map_set_pairs:
                map_files = [
                    meta.get_map_filename_transfer2(id_sim, cl_type=cl_type, map_set=ms)
                    for ms in map_sets
                ]

                if filter_flag == "filtered":
                    map_files = [
                        mf.replace(".fits", "_filtered.fits") for mf in map_files
                    ]

                maps = [
                    mu.read_map(m, field=[0, 1, 2], convert_K_to_muK=True)
                    for m in map_files
                ]
                ...
                field = [
                    {
                        ...
                        "spin2": nmt.NmtField(
                            mask, map[1:], purify_b=meta.tf_est_pure_B
                        ),
                    }
                    for map in maps
                ]
                ...
                cl_suffix = (
                    f"_{map_sets[0]}_{map_sets[1]}"
                    if meta.validate_beam
                    else f"_{filter_flag}"
                )
                cl_name = cl_prefix + cl_suffix

                np.savez(
                    f"{meta.cell_transfer_directory}/{cl_name}.npz", **decoupled_pcls
                )
...
def get_binned_cls(bp_win_dict, cls_dict_unbinned):
    ...
    for spin_comb in ["spin0xspin0", "spin0xspin2", "spin2xspin2"]:
        ...
        if spin_comb == "spin0xspin0":
            ...
            cls_vec = np.array(
                [cls_dict_unbinned["TE"][:nl], cls_dict_unbinned["TB"][:nl]]
            )
        elif spin_comb == "spin2xspin2":
            cls_vec = np.array(
                [
                    cls_dict_unbinned["EE"][:nl],
...
def get_pcls_mat_transfer(fields, nmt_binning, fields2=None):
    ...
    for index, (pure_type1, pure_type2) in enumerate(product(cases, cases)):
        pcls = get_coupled_pseudo_cls(
            fields[pure_type1], fields2[pure_type2], nmt_binning
        )
