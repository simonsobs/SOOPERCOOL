...
cl = 1 / (ls + 10) ** 2
np.savez("PL/cl_PL.npz", ls=ls, clEE=cl, clEB=cl, clBE=cl, clBB=cl)

clEE = 1 / (ls + 10) ** 0.5
clBB = 0.05 / (ls + 10) ** 0.5
cl0 = np.zeros(3 * nside)
np.savez("val/cl_val.npz", ls=ls, clEE=clEE, clEB=cl0, clBE=cl0, clBB=clBB)
...
np.savez("CMBl/cl_CMBl.npz", ls=ls, clEE=clEE_CMBl, clEB=cl0, clBE=cl0, clBB=clBB_CMBl)

dlr = np.loadtxt("camb_lens_r1.dat", unpack=True)
rfid = 0.01
clEE_CMBr = clEE_CMBl
clBB_CMBr = np.zeros(3 * nside)
clBB_CMBr[1:] = (2 * np.pi * rfid * (dlr[3] - d[3]) / (d[0] * (d[0] + 1)))[
    : 3 * nside - 1
]
np.savez("CMBr/cl_CMBr.npz", ls=ls, clEE=clEE_CMBr, clEB=cl0, clBE=cl0, clBB=clBB_CMBr)
