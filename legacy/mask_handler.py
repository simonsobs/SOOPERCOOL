...
def mask_handler(args):
    ...
    binary_mask = get_binary_mask_from_nhits(nhits, meta.nside, zero_threshold=1e-3)
    ...
    if args.plots:
        ...
        hp.graticule()
        plt.savefig(
            os.path.join(plot_dir, meta.masks["binary_mask"]).replace(".fits", ".png")
        )
    ...
    if not meta.use_input_nhits:
        ...
        if "galactic" in meta.masks["include_in_mask"]:
            ...
            for id_key, gal_key in enumerate(gal_keys):
                ...
                meta.timer.stop(
                    f"gal{id_key}", f"Galactic mask {gal_key} projection", args.verbose
                )
                ...
                if args.plots:
                    ...
                    plt.savefig(
                        os.path.join(
                            plot_dir, fname.split("/")[-1].replace(".fits", ".png")
                        )
                    )
        ...
        if "point_source" in meta.masks["include_in_mask"]:
            ...
            if os.path.isfile(ps_fname):
                ...
                meta.timer.stop(
                    "ps_mask", "Load point source mask from disk", args.verbose
                )
            # Otherwise, generate random point source mask
                ...
                meta.timer.stop(
                    "ps_mask", "Generate mock point source mask", args.verbose
                )
            ...
            if args.plots:
                ...
                plt.savefig(
                    os.path.join(
                        plot_dir, ps_fname.split("/")[-1].replace(".fits", ".png")
                    )
                )
        ...
        first_is_bounded = np.amax(first) < 2 * np.amax(first_nom) and np.amin(
            first
        ) > 2 * np.amin(first_nom)
    ...
    if args.plots:
        ...
        hp.graticule()
        plt.savefig(
            os.path.join(plot_dir, meta.masks["analysis_mask"]).replace(".fits", ".png")
        )
        plt.clf()
        ...
        hp.graticule()
        plt.savefig(
            os.path.join(plot_dir, meta.masks["analysis_mask"]).replace(
                ".fits", "_first.png"
            )
        )
        plt.clf()
