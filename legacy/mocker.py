...
def mocker(args):
    ...
    lth, nlth_deconvolved_dict = get_noise_cls(
        noise_kwargs=meta.noise, lmax=lmax_sim, fsky=fsky, is_beam_deconvolved=False
    )
    ...
    for ms in meta.map_sets_list:
        ...
        np.savez(
            f"{meta.mock_directory}/noise_{ms}.npz",
            lth=lth,
            **nlth_deconvolved_dict[ftag],
        )
    meta.timer.stop("Computing noise cls")

    # Load beams for each map set
    beam_windows = {
        map_set: meta.read_beam(map_set)[1] for map_set in meta.map_sets_list
    }
    ...
    for id_sim in mpi_utils.taskrange(Nsims - 1):
        alms_T, alms_E, alms_B = hp.synalm(
            [ps_th[k] for k in hp_ordering], lmax=lmax_sim
        )
        if meta.null_e_modes:
            cmb_map = hp.alm2map(
                [alms_T, alms_E * 0, alms_B], meta.nside, lmax=lmax_sim
            )
        else:
            cmb_map = hp.alm2map([alms_T, alms_E, alms_B], meta.nside, lmax=lmax_sim)

        for map_set in meta.map_sets_list:

            meta.timer.start(f"Generate map set {map_set} split maps")
            cmb_map_beamed = hp.smoothing(cmb_map, beam_window=beam_windows[map_set])
            ...
            for id_split in range(n_splits):
                if "SAT" in map_set:
                    ...
                    noise_map = to_muk * hp.read_map(fname_noise, field=[0, 1, 2])
                ...
                map_file_name = meta.get_map_filename(
                    map_set, id_split, id_sim if Nsims > 1 else None
                )
                hp.write_map(map_file_name, split_map, overwrite=True, dtype=np.float32)

                if args.plots:
                    if Nsims == 1:
                        plot_dir = meta.plot_dir_from_output_dir(meta.map_directory_rel)
                        utils.plot_map(
                            split_map,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--sims", action="store_true", help="Generate a set of sims if True."
    )
    parser.add_argument(
        "--plots", action="store_true", help="Plot the generated maps if True."
    )
    ...
    if args.sims and args.plots:
        warnings.warn(
            "Both --sims and --plot are set to True. "
